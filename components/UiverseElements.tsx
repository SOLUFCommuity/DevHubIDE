...
export const PluginCard: React.FC<{ plugin: { name: string, description: string, version: string, installed: boolean }, onInstall: () => void }> = ({ plugin, onInstall }) => (
  <div className="bg-[#161b22] border border-[#30363d] p-3 rounded-lg hover:border-blue-500/40 transition-all group relative overflow-hidden">
    <div className="flex justify-between items-start mb-1">
      <h4 className="text-xs font-bold text-gray-200">{plugin.name}</h4>
      <span className="text-[9px] text-gray-600 bg-gray-800/50 px-1.5 py-0.5 rounded">v{plugin.version}</span>
    </div>
    <p className="text-[10px] text-gray-500 mb-3 line-clamp-2">{plugin.description}</p>
    <button 
      onClick={onInstall}
...
export const SearchResultItem: React.FC<{ fileName: string, line: number, text: string, onClick: () => void }> = ({ fileName, line, text, onClick }) => (
  <button 
    onClick={onClick}
    className="w-full text-left p-2 rounded-md hover:bg-[#1f242b] transition-all border border-transparent hover:border-[#30363d] group"
  >
    <div className="flex items-center justify-between mb-1">
      ...
      <span className="text-[10px] text-gray-500 opacity-0 group-hover:opacity-100 transition-opacity font-mono">Line {line}</span>
    </div>
    <div className="text-[10px] text-gray-400 font-mono truncate pl-4 border-l border-[#30363d] ml-1">
      {text.trim()}
    </div>
  </button>
);
